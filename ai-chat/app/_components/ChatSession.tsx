...
import type { Agent, ChatSession as ChatSessionType } from "../_types"
...
const ChatSession = memo(
  ({
    ...
    const handleReset = contextSafe(() => {
      ...
      gsap.to(containerRef.current, {
        ...
        onComplete: () => {
          ...
          // リセット後に初期状態を復元
          gsap.set(
            [headerRef.current, messagesRef.current, footerRef.current],
            {
              opacity: 1,
              y: 0,
              scale: 1,
    ...
    const getRoundProgress = () => {
      return Math.min(
        (chatSession.currentRound / chatSession.maxRounds) * 100,
        100
      )
    }
      ...
      <div ref={containerRef} className="w-full max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row gap-6 h-full">
          ...
          <div className="w-full lg:w-2/5 space-y-6">
            ...
            <div
              ref={headerRef}
              className="glass-morphism rounded-3xl p-6 shadow-2xl"
            >
                  ...
                  <Card className="bg-gradient-to-br from-green-50 to-emerald-50 border-green-200/50 shadow-lg">
                    <CardContent className="pt-6">
                      ...
                      <p className="text-green-700">
                        {chatSession.maxRounds}
                        ラウンドのディスカッションが完了しました。左側に最終結論が表示されています。
                      </p>
                    </CardContent>
                  </Card>
                </div>
